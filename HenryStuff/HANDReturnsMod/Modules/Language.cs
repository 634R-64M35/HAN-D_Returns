...
namespace HANDReturns.Modules {
    internal static class Language
    {
        ...
        private static void Language_collectLanguageRootFolders(List<string> obj) {
            string path = Path.Combine(Path.GetDirectoryName(HANDReturnsPlugin.instance.Info.Location), "Languages");
        ...
        public static void PrintOutput(string fileName = "") {
            ...
            if (!string.IsNullOrEmpty(fileName)) {
                string path = Path.Combine(Directory.GetParent(HANDReturnsPlugin.instance.Info.Location).FullName, "Languages", fileName);
